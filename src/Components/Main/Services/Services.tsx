...
const Services = () => {
  ...
  const sid = selectedServiceData?.id;
  const heading =
    sid !== undefined ? t(`services.items.${sid}.heading`) : '';
  const texts =
    sid !== undefined
      ? (t(`services.items.${sid}.texts`, {
          returnObjects: true,
        }) as string[])
      : [];
  ...
  return (
    <SectionWrapper id="specialites">
      ...
      >
        <h2 className="text-4xl font-bold mb-2">{t('services.sectionTitle')}</h2>
        <p className="opacity-80 mb-3">{t('services.intro1')}</p>
      ...
      <div className="flex flex-col justify-between lg:flex-row gap-10 lg:gap-5">
        ...
        >
          <h3 className="text-xl xs:text-2xl font-bold mb-3">{heading}</h3>
          <p className="opacity-80 mb-5 text-sm md:text-base">
            {t('services.moreInfo')}
          </p>
          <ul className="lg:ms-5 min-h-[310px] sm:min-h-[225px] md:min-h-[190px] lg:min-h-[375px] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-1 gap-4 md:gap-6">
            {texts.map((text, index) => (
              <ListItem key={index} text={text} />
            ))}
          </ul>
          <div className="lg:ms-5 mt-6">
